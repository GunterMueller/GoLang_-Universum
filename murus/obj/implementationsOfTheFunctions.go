...
func eq (a, b Any) bool {
  ...
  case []bool:
    n := len(a.([]bool))
    if n != len(b.([]bool)) { return false }
    for i, y := range a.([]bool) {
      if b.([]bool)[i] != y {
        return false
      }
      return true
  ...
  case []byte:
    n := len(a.([]byte))
    if n != len(b.([]byte)) { return false }
    for i, y := range a.([]byte) {
      if b.([]byte)[i] != y {
        return false
      }
      return true
...
func clone (a Any) Any {
  ...
  case []bool:
    ...
    return b
  default:
    ker.Panic ("murus only clones atomic types and objects of type []bool, []byte or Equaler")
  }
  return nil
}
...
func atomic (a Any) bool {
  ...
  case bool,
       ...
       string, []byte: // we treat them also as atomic
    return true
  }
  return false
}
...
func fail (a Any) {
  ker.Panic ("murus only [en|de]codes atomic types " +
             "and objects of type string, []byte, []bool or Coder !")
}

...
// (c) murus.org  v. 140415 - license see murus.go
...
func (X *window) Write1 (b byte, l, c uint) {
  X.Write (z.String(b), l, c)
}

func (X *window) Write (s string, l, c uint) {
  X.WriteGr (s, int(c) * int(X.wd1), int(l) * int(X.ht1))
}

func (X *window) WriteNat (n uint, l, c uint) {
  t:= "00"
  if n > 0 {
    const M = 10
    bs:= make ([]byte, M)
...
func (X *window) Write1Gr (b byte, x, y int) {
  X.WriteGr (z.String(b), x, y)
}

func (X *window) WriteGr (s string, x, y int) {
...
func (X *window) Write1InvGr (b byte, x, y int) {
  X.WriteInvGr (z.String(b), x, y)
}

func (X *window) WriteInvGr (s string, x, y int) {
  C.XSetFunction (dpy, X.gc, C.GXinvert)
  X.WriteGr (s, x, y)
  C.XSetFunction (dpy, X.gc, C.GXcopy)
}

func (X *window) Transparent() bool {
  return X.transparent
}

func (X *window) Transparence (t bool) {
  X.transparent = t
}

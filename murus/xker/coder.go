...
func (X *window) Codelen (w, h uint) uint {
  return 4 * (4 + w * h)
}

func (X *window) Encode (x, y, w, h uint) []byte {
  if w == 0 || h == 0 || w > uint(X.wd) || h > uint(X.ht) { panic ("xker.Encode: w or h has wrong value") }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////  steals a lot of time
  ...
  C.XPutImage (dpy, C.Drawable(X.win), X.gc, ximg, nul, nul, C.int(x), C.int(y), C.uint(w), C.uint(h))

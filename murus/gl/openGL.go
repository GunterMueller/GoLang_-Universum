...
/*
  ...
  class = POINTS
  C.glBegin (class)
if tst { println ("glBegin", text[class]) }
*/
  nn = 0
//  counter = 1
}
...
//              ^^^ at start: POINTS
      ...
      nn = a
      C.glEnd()
if tst { println (a, "glEnd") }
      C.glBegin (C.GLenum(class))
if tst { println (a, "glBegin", text[class]) }
    }
    C.glColor3ub (C.GLubyte(c.R), C.GLubyte(c.G), C.GLubyte(c.B))
if tst { println ("class", text[class]) }
    for i := uint(0); i < a; i++ {
      vector2yyy (V[i])
if tst { println (i, V[i].String()) }
      C.glVertex3dv (&yyy[0])

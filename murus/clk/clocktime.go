...
func (x *clocktime) Edit(l, c uint) {
  ...
  for {
    ...
    if x.Defined(s) {
      ...
      case 2:
        errh.Error0("Bitte wiederholen Sie die Uhrzeitangabe, sie ist immer noch unverständlich !") // , l + 1, c)
      case 3:
        errh.Error0("Jetzt passen Sie doch gefälligst auf !") // l + 1, c)
      case 4:
        errh.Error0("Was soll der Quatsch? Ist das eine Uhrzeit ?") // l + 1, c)
      case 5:
        errh.Error0("Schaffen Sie es nicht, eine Uhrzeit richtig einzugeben ?") // l + 1, c)
      default:
        errh.Error0("Vergessen Sie's ...") // , l + 1, c)
        x.Actualize()
        return
      }
    }

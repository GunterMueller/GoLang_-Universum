...
// #include <GL/glx.h>
/*
int eventtyp (XEvent *e) { return (*e).type; }
...
func new_(x, y uint, m mode.Mode) XWindow {
  ...
  a := [12]C.int{ 4, 8, 1, 9, 1, 10, 1, 12, 12, 5, 0 }

  vi := C.glXChooseVisual (dpy, screen, &a[0] )
  cx := C.glXCreateContext (dpy, vi, C.GLXContext(nil), C.True)
//  C.glXMakeContextCurrent (dpy, C.GLXDrawable(X.win), C.GLXDrawable(X.win), cx) XXX hier nicht !
  ...
  swa.border_pixel = 8
  swa.event_mask = C.ExposureMask | C.ButtonPressMask | C.StructureNotifyMask
/*
  X.win = C.XCreateSimpleWindow (dpy, C.Window(rootWin), C.int(x), C.int(y), X.wd, X.ht, 0,
                                 cc (X.scrF), cc (X.scrB))
*/
  X.win = C.XCreateWindow (dpy, rootWin, C.int(x), C.int(y), X.wd, X.ht,
                           0, vi.depth, C.InputOutput, vi.visual,
                           C.CWBorderPixel | C.CWColormap | C.CWEventMask, &swa)
  X.gc = C.XDefaultGC (dpy, screen)
  C.XSetGraphicsExposures (dpy, X.gc, C.False)
/*
  n := C.int(0)
  config := *(C.glXChooseFBConfig (dpy, screen, &a[0], &n))
  c := C.glXCreateNewContext (dpy, config, C.GLX_RGBA_TYPE, C.GLXContext(nil), 1)
  C.glXMakeContextCurrent (dpy, C.GLXDrawable(X.win), C.GLXDrawable(X.win), c)
*/
  C.glXMakeContextCurrent (dpy, C.GLXDrawable(X.win), C.GLXDrawable(X.win), cx) // XXX hier !
...
func imp (w C.Window) *xwindow {
  ...
  panic ("murus/xwin.imp: there is no xwindow there")
}
...
func Win (i uint) *xwindow {
  ...
  panic ("murus/xwin.Win: there is no xwindow there with number" + strconv.Itoa(int(i)))
}
*/

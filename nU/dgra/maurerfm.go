...
func (x *distributedGraph) m (a any, i uint) any {
  ...
  if x.me != x.root {
    x.tmpGraph.Ex2 (x.nr[j], x.actVertex)
    x.cycle.Edge (x.tmpGraph.Get1())
    if x.nr[out] == x.root {
      x.cycle.Ex2 (x.actVertex, x.nr[out])
      x.tmpGraph.Ex2 (x.actVertex, x.nr[out])
      x.cycle.Edge (x.tmpGraph.Get1())
    }
    x.cycle.Write()
    bs = x.cycle.Encode()
...
func (x *distributedGraph) Maurerfm() {
  ...
  go func() {
    fmon.New (nil, 1, x.m, AllTrueSp, x.actHost, p0 + uint16(x.me), true)
  }()
  for i := uint(0); i < x.n; i++ {
    x.mon[i] = fmon.New (nil, 1, x.m, AllTrueSp, x.host[i], p0 + uint16(x.nr[i]), false)
  }
  defer x.finMon()
  x.awaitAllMonitors()

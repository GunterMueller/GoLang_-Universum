...
// (c) Christian Maurer   v. 201018 - license see µU.go
...
func (X *xwindow) Decode (s obj.Stream) {
  ...
  ximg := C.XGetImage (dpy, C.Drawable(X.win), C.int(x), C.int(y),
                       C.uint(w), C.uint(h), M, C.XYPixmap) // steals a lot of time
  var pixel C.ulong
  for j := uint32(0); j < uint32(h); j++ {
    for i := uint32(0); i < uint32(w); i++ {
      pixel = (C.ulong)(obj.Decode (n, s[n:n+4]).(int32))
      C.xPutPixel (ximg, C.int(i), C.int(j), pixel)
      n += 4
    }
  }
  C.XPutImage (dpy, C.Drawable(X.win), X.gc, ximg, 0, 0, C.int(x), C.int(y), C.uint(w), C.uint(h))
  C.XCopyArea (dpy, C.Drawable(X.win), C.Drawable(X.buffer), X.gc, C.int(x),
               C.int(y), C.uint(w), C.uint(h), C.int(x), C.int(y))

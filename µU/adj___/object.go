...
func (x *adjacencyMatrix) Copy (Y any) {
  y := x.imp (Y)
  x.uint = y.uint
	x.e, x.v = Clone(y.e).(edg.Edge), Clone(y.v).(vtx.Vertex)
  for i := uint(0); i < x.uint; i++ {
    for k := uint(0); k < x.uint; k++ {
      x.entry[i][k].vertex = y.entry[i][k].vertex.Clone().(vtx.Vertex)
...
func (x *adjacencyMatrix) Decode (bs Stream) {
  ...
  i := uint(4)
  x.v = Decode (x.v, bs[i:i+v]).(vtx.Vertex)
  i += v
  x.e = Decode (x.e, bs[i:i+e]).(edg.Edge)
  i += e
  for j := uint(0); j < x.uint; j++ {
    for k := uint(0); k < x.uint; k++ {
      x.entry[j][k].vertex = Decode (x.v, bs[i:i+v]).(vtx.Vertex)
      i += v
      x.entry[j][k].edge = Decode (x.e, bs[i:i+e]).(edg.Edge)
      i += e
    }
  }
}

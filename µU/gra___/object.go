...
func (x *graph) Eq (Y any) bool { // disgusting complexity
  ...
  if x.nVertices != y.nVertices || x.nEdges != y.nEdges ||
     ! TypeEq (x.vAnchor.v, y.vAnchor.v) ||
     ! TypeEq (x.eAnchor.e, y.eAnchor.e) {
    return false
  }
  ya := y.local // save
  eq := true
      ...
      for yn := yv.nbPtr; yn.nextNb != yv.nbPtr; yn = yn.nextNb {
        if yn.to == xn.to {
          ...
          if x.eAnchor.e != nil {
            ...
            aa = Eq (xn.edgePtr.e, yn.edgePtr.e)
          }
          if aa {
            break // next xnb
          } else {
...
func (x *graph) Clone() any {
  y := new_(x.bool, x.vAnchor.v, x.eAnchor.e)
  y.Copy (x)
  return y
}
...
func (x *graph) Encode() Stream {
  ...
  z := uint32(0)
  for v := x.vAnchor.nextV; v != x.vAnchor; v = v.nextV {
    k := uint32(Codelen (v.v))
    copy (s[i:i+a], Encode (k))
    i += a
    s[i] = 0; if v.bool { s[i] = 1 }
    i++
    copy (s[i:i+k], Encode (v.v))
    i += k
    v.dist = z
    z++
  }
  ...
  i += a
  for e := x.eAnchor.nextE; e != x.eAnchor; e = e.nextE {
    if x.eAnchor.e == nil { ker.Oops() }
    k := uint32(Codelen (e.e))
    copy (s[i:i+a], Encode (k))
    i += a
    copy (s[i:i+k], Encode (e.e))
    i += k
...
func (x *graph) Decode (s Stream) {
  ...
  for n := uint32(0); n < x.nVertices; n++ {
    ...
    marked := s[i] == 1
    i++
    vc := x.vAnchor.v.Clone().(vtx.Vertex)
    v := Decode (x.vAnchor.v, s[i:i+k]).(vtx.Vertex)
    x.vAnchor.v = vc.Clone().(vtx.Vertex)
    x.insertedVertex (v, marked)
    i += k
  }
  ...
  for z := uint32(0); z < x.nEdges; z++ {
    k := Decode (uint32(0), s[i:i+a]).(uint32)
    i += a
    attrib := Decode (x.eAnchor.e, s[i:i+k]).(edg.Edge)
    e := newEdge (attrib)
    i += k
    e.bool = s[i] == 1
    i++

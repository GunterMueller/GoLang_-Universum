...
// float64 = ["-"] digit {digit} [ [","|"."] [ digit {digit} ] ] ["e" ["-"] digit {digit} ]
  ...
  if p[0] > 0 {
    if p[0] != 1 {
      ...
      ker.Panic ("invalid character after the first one")
    }
    if s[0] == '-' {
      x = -x
    } else {
      fail ("invalid first character", 0)
      ker.Panic ("invalid first character")
    }
  }
  if k == 1 {

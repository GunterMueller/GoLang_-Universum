...
func line (c col.Colour, x []float64) {
  ...
  gl.Begin (gl.LINES)
  gl.Colour (c)
  gl.Vertex (x[0], x[1], x[2])
  gl.Vertex (x[3], x[4], x[5])
  gl.End()
}
...
func vertRectangle (x []float64) {
  ...
  gl.Vertex (x[0], x[1], x[2])
  gl.Vertex (x[3], x[4], x[2])
  gl.Vertex (x[3], x[4], x[5])
  gl.Vertex (x[0], x[1], x[5])
}
...
func parallelogram (c col.Colour, x []float64) {
  ...
  gl.Colour (c)
  gl.Vertex (x[0],               x[1],               x[2])
  gl.Vertex (x[0] + x[3],        x[1] + x[4],        x[2] + x[5])
  gl.Vertex (x[0] + x[3] + x[6], x[1] + x[4] + x[7], x[2] + x[5] + x[8])
  gl.Vertex (x[0] + x[6],        x[1] + x[7],        x[2] + x[8])
  gl.End()
}
...
func prism (c col.Colour, x []float64) {
  ...
  for i := 3; i < n - 3; i += 3 {
    gl.Vertex (x [i], x [i + 1], x [i + 2])
    gl.Vertex (x1[i], x1[i + 1], x1[i + 2])
  }
  gl.Vertex (x [3], x [4], x [5])
  gl.Vertex (x1[3], x1[4], x1[5])
  gl.End()
}
...
func prismC (c []col.Colour, x []float64) {
  ...
  for i := 3; i < n - 3; i += 3 {
    j := i / 3 - 1
    gl.VertexC ( x[i]    ,  x[i + 1],  x[i + 2], c[j])
    gl.VertexC ( x[i + 3],  x[i + 4],  x[i + 5], c[j])
    gl.VertexC (x1[i + 3], x1[i + 4], x1[i + 5], c[j])
    gl.VertexC (x1[i],     x1[i + 1], x1[i + 2], c[j])
  }
  j := n / 3 - 1
  gl.VertexC (x [n - 3], x [n - 2], x [n - 1], c[j])
  gl.VertexC (x [3],     x [4],     x [5],     c[j])
  gl.VertexC (x1[3],     x1[4],     x1[5],     c[j])
  gl.VertexC (x1[n - 3], x1[n - 2], x1[n - 1], c[j])
  gl.End()
}
...
func pyramidC (c []col.Colour, x, y, z, a, b, h float64) {
  gl.Begin (gl.TRIANGLE_FAN)
  gl.VertexC (x,     y,     z + h, c[0])
  gl.VertexC (x + a, y + b, z,     c[0])
  gl.VertexC (x - a, y + b, z,     c[0])
  gl.VertexC (x - a, y - b, z,     c[1])
  gl.VertexC (x + a, y - b, z,     c[2])
  gl.VertexC (x + a, y + b, z,     c[3])
  gl.End()
  gl.Begin (gl.QUADS)
...
func octahedronC (c []col.Colour, x, y, z, r float64) { // BUG
  ...
  gl.Begin (gl.TRIANGLES)
  gl.Colour (c[0])
  gl.Vertex (x, y, z + d); gl.Vertex (x + r, y - r, z); gl.Vertex (x + r, y + r, z)
  gl.Colour (c[1])
  gl.Vertex (x, y, z + d); gl.Vertex (x + r, y + r, z); gl.Vertex (x - r, y + r, z)
  gl.Colour (c[2])
  gl.Vertex (x, y, z + d); gl.Vertex (x - r, y + r, z); gl.Vertex (x - r, y - r, z)
...
//  if r < 0 { r = -r } // change orientation
  ...
  gl.Vertex (x, y, z)
  for i := 0; i <= N; i++ {
    gl.Vertex (x + r * cos[i], y + r * sin[i], z)
  }
  gl.End()
}
...
//  if r < 0 { r = -r } // change orientation
  ...
  gl.Vertex (x, y, z)
  for i := 0; i <= N; i++ {
    gl.Vertex (x - r * s * cos[i], y + r * c * cos[i], z + r * sin[i])
  }
  gl.End()
}
...
func sphere (f col.Colour, x, y, z, r float64) {
  ...
  gl.Vertex (x, y, z + r)
  for i := 0; i <= N; i++ {
    gl.Vertex (x + r0 * cos[i], y + r0 * sin[i], z0)
  }
  gl.End()
  gl.Begin (gl.QUAD_STRIP)
  for i := 1; i <= N / 2 - 2; i++ {
    ...
    for j := 0; j <= N; j++ {
      s, c := sin[j], cos[j]
      gl.Vertex (x + r0 * c, y + r0 * s, z0)
      gl.Vertex (x + r1 * c, y + r1 * s, z1)
    }
  }
  gl.End()
...
func cone (c col.Colour, x, y, z, r, h float64) {
  ...
  gl.Vertex (x, y, z + h)
  for l := 0; l <= N; l++ {
    gl.Vertex (x + r * cos[l], y + r * sin[l], z)
/*
  n := vectors(N + 2)
  n[0] = vect.New3 (0, 0, 1)
  for l := 0; l <= N; l++ {
...
//  cylinderSegment (c, x, y, z, r, h, 0, 360)
  ...
  for i := 0; i <= N; i++ {
    gl.Vertex (x + r * cos[i], y + r * sin[i], z)
    gl.Vertex (x + r * cos[i], y + r * sin[i], z + h)
  }
  gl.End()
}
...
func torus (c col.Colour, x, y, z, R, r float64) {
  ...
  for i := 0; i < N; i++ {
    ...
    for j := 0; j < N; j++ {
      gl.Vertex (x + s0 * cos[j],     y + s0 * sin[j],     z0)
      gl.Vertex (x + s0 * cos[j + 1], y + s0 * sin[j + 1], z0)
      gl.Vertex (x + s1 * cos[j + 1], y + s1 * sin[j + 1], z1)
      gl.Vertex (x + s1 * cos[j],     y + s1 * sin[j],     z1)
      gl.Vertex (x + s0 * cos[j],     y + s0 * sin[j],     z0)
//      n[2*j].Set3 (1., 1., 1.)
      gl.Vertex (x + s0 * cos[j + 1], y + s0 * sin[j + 1], z0)
//      n[2*j+1].Set3 (1, 1, 1)
...
func horTorus (c col.Colour, x, y, z, R, r, a float64) { // XXX a ?
  ...
  for i := 0; i < N; i++ {
    ...
    for j := 0; j < N; j++ { //  x -> x * c - y * sa, y -> x * sa + y * ca
      ...
      gl.Vertex (x + x0 * ca - y00 * sa, y + x0 * sa + y00 * ca, z + s0 * sin[j])
      gl.Vertex (x + x0 * ca - y01 * sa, y + x0 * sa + y01 * ca, z + s0 * sin[j + 1])
      gl.Vertex (x + x1 * ca - y11 * sa, y + x1 * sa + y11 * ca, z + s1 * sin[j + 1])
      gl.Vertex (x + x1 * ca - y10 * sa, y + x1 * sa + y10 * ca, z + s1 * sin[j])
    }
  }
  gl.End()
}
...
//      }
  ...
  figure (gl.LINES, cX, -G1, N, N), G1, N, N))
  sphere (G1, N, N, R1, cX)
  if mit {
...
//      }
  ...
  figure (gl.LINES, cY, N,-G1, N), N, G1, N))
  sphere (N, G1, N, R1, cY)
  if mit {
...
//      }
  ...
  figure (gl.LINES, cZ, N, N,-G1), N, N, G1))
  sphere (N, N, G1, R1, cZ)
*/
}
